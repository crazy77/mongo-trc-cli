...
import { fileURLToPath } from "node:url"; // Needed for __dirname in NodeNext/ESM
...
import {
	...
	updateSchemaDerivedFile,
} from "./utils/fileUpdater.js";
...
async function runGenerateLogic(options: {
	...
	generateSpinner.info(
		`Schema file: ${path.relative(process.cwd(), schemaPath)}`,
	);
	generateSpinner.info(
		`Derived file: ${path.relative(process.cwd(), derivedPath)}`,
	);
	generateSpinner.info(
		`API output directory: ${path.relative(process.cwd(), outputDir)}`,
	);
	generateSpinner.info(
		`tRPC helper path: ${path.relative(process.cwd(), trpcPath)}`,
	);
	generateSpinner.info(
		`Util file path (derived): ${path.relative(process.cwd(), utilPath)}`,
	);
	...
	try {
		...
		generateSpinner.succeed(
			`Generated API files for: ${schemaNames.join(", ")}`,
		);
...
program.description(
	"A CLI tool for managing Mongoose models and APIs based on Zod schemas",
);
...
program
	...
	.action(async (options) => {
		...
		try {
			...
			const dbFilesToProcess = [
				"connection.ts",
				"schema.ts",
				"schemaDerived.ts",
				"util.ts",
			];
			const derivedSchemaTargetPath = path.join(
				dbOutputDir,
				"schemaDerived.ts",
			);
			...
			for (const dbFile of dbFilesToProcess) {
				...
				const fileExists = await fsExtra.pathExists(targetPath);

				if (!(await fsExtra.pathExists(templatePath))) {
					initSpinner.fail(
						`❌ Error: DB Template file not found at ${templatePath}.`,
					);
					continue;
				}
				...
				if (!fileExists || forceOverwrite) {
					if (dbFile === "util.ts") {
						...
						const relativePath = getRelativeImportPath(
							targetPath,
							derivedSchemaTargetPath,
						);
						const processedContent = templateContent.replace(
							"%%RELATIVE_PATH_TO_DERIVED%%",
							relativePath,
						);
					...
					initSpinner.info(
						`    Skipping ${dbFile} (exists and --force not used).`,
					);
				}
			}

			initSpinner.succeed(
				"Initialization complete! Review copied/processed files.",
			);
		} catch (error: unknown) {
...
program
	.command("generate")
	.description(
		"Generate API files and update schemaDerived based on schema.ts.",
	)
	.option(
		"--schema-path <path>",
		"Path to the Zod schema definition file",
		"./src/server/db/schema.ts",
	)
	.option(
		"--derived-path <path>",
		"Path to the derived schema and types file",
		"./src/server/db/schemaDerived.ts",
	)
	.option(
		"-o, --output-dir <path>",
	...
	.option(
		"--trpc-path <path>",
		"Path to the tRPC helper file (trpc.ts)",
...
program
	.command("watch")
	.description(
		"Watch the schema file for changes and automatically run generate.",
	)
	...
	.action(async (options) => {
		...
		watcher
			.on("add", async (filePath) => {
				...
				console.log(
					`➕ File ${path.basename(filePath)} has been added. Triggering generate...`,
				);
				await runGenerateLogic(options);
			})
			.on("change", async (filePath) => {
				console.log(
					`🔄 File ${path.basename(filePath)} has been changed. Triggering generate...`,
				);
				await runGenerateLogic(options);
			})
