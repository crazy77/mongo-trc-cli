...
import type { getManySchema } from "./schemaDerived.js";
...
export const getMany = async <T>(
	model: Model<T>,
	input: z.infer<typeof getManySchema>,
	...
	let currentQueryConditions: FilterQuery<T> = {
		...baseQueryConditions,
	} as FilterQuery<T>;
	if (cursor) {
		...
		const cursorValueCondition = {
			[sortBy]: sort === "desc" ? { $lt: cursor } : { $gt: cursor },
		} as FilterQuery<T>;
		currentQueryConditions = {
			...baseQueryConditions,
			...cursorValueCondition,
		};
	}
	...
	const lastItemSortFieldValue =
		(lastItem as unknown as Record<string, string | Date | number>)?.[sortBy] ??
		null;

	let hasMore = false;
	if (
		lastItemSortFieldValue !== null &&
		data.length > 0 &&
		data.length === limit
	) {
		const hasMoreQueryConditions: FilterQuery<T> = {
			...
			[sortBy]:
				sort === "desc"
					? { $lt: lastItemSortFieldValue }
					: { $gt: lastItemSortFieldValue },
		} as FilterQuery<T>;
		hasMore =
			(await model
				.findOne(hasMoreQueryConditions)
				.select({ _id: 1 })
				.lean()) !== null;
	}

	const total = await model.countDocuments(
		baseQueryConditions as FilterQuery<T>,
	);
